...
class GaussianProcessTests extends ScalismoTestSuite {
  ...
  describe("a discrete LowRank Gaussian process") {
    ...
    it("yeilds the same result when marginalized the points in one or two steps") {
      ...
      DiscreteField.vectorize[_3D, EuclideanVector[_3D]](dgp2.mean) should equal(DiscreteField.vectorize[_3D, EuclideanVector[_3D]](dgp3.mean))
    ...
    it("yields the same result for the marginal as the discrete gp") {
      ...
      DiscreteField.vectorize[_3D, EuclideanVector[_3D]](dgp1.mean) should equal(DiscreteField.vectorize[_3D, EuclideanVector[_3D]](dgp2.mean))
