...
case class ActiveShapeModel(statisticalModel: StatisticalMeshModel, profiles: Profiles, preprocessor: ImagePreprocessor, featureExtractor: FeatureExtractor) {
  ...
  def mean(): ASMSample = {
    ...
    val featureField = DiscreteFeatureField[_3D](new UnstructuredPointsDomain3D(meanProfilePoints), meanFeatures)
  ...
  def sample()(implicit rand: Random): ASMSample = {
    ...
    val featureField = DiscreteFeatureField[_3D](new UnstructuredPointsDomain3D(randomProfilePoints), randomFeatures)
  ...
  def sampleFeaturesOnly()(implicit rand: Random): ASMSample = {
    ...
    val meanProfilePoints = profiles.data.map(p => smean.pointSet.point(p.pointId))
    val randomFeatures = profiles.data.map(_.distribution.sample)
    val featureField = DiscreteFeatureField[_3D](new UnstructuredPointsDomain3D(meanProfilePoints), randomFeatures)
    ASMSample(smean, featureField, featureExtractor)
  }
