...
import scalismo.common.{PointId, UnstructuredPoints, UnstructuredPoints3D}
...
case class ASMSample(mesh: TriangleMesh[_3D],
                     featureField: DiscreteFeatureField[_3D, UnstructuredPoints[_3D]],
                     featureExtractor: FeatureExtractor)
...
case class ActiveShapeModel(statisticalModel: StatisticalMeshModel,
  ...
  def mean(): ASMSample = {
    ...
    val featureField = DiscreteFeatureField[_3D, UnstructuredPoints[_3D]](
      new UnstructuredPoints3D(meanProfilePoints),
      meanFeatures
    )
  ...
  def sample()(implicit rand: Random): ASMSample = {
    ...
    val featureField = DiscreteFeatureField[_3D, UnstructuredPoints[_3D]](
      new UnstructuredPoints3D(randomProfilePoints),
      randomFeatures
    )
