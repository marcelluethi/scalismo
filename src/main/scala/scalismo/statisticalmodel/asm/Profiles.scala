...
class DiscreteFeatureField[D: NDSpace](domain: DiscreteDomain[D], _values: IndexedSeq[DenseVector[Double]])
...
object DiscreteFeatureField {
  def apply[D: NDSpace](domain: DiscreteDomain[D], values: IndexedSeq[DenseVector[Double]]) = new DiscreteFeatureField[D](domain, values)
}
