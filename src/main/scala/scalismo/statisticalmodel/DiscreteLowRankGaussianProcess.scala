...
case class DiscreteLowRankGaussianProcess[D <: Dim: NDSpace, Dom <: DiscreteDomain[D], Value] private[scalismo] (_domain: Dom, meanVector: DenseVector[Double], variance: DenseVector[Double], basisMatrix: DenseMatrix[Double])(override implicit val vectorizer: Vectorizer[Value])
    extends DiscreteGaussianProcess[D, Dom, Value](DiscreteField.createFromDenseVector[D, Dom, Value](_domain, meanVector), basisMatrixToCov(_domain, variance, basisMatrix)) {
  self =>
   ...
   */
  def instance(c: DenseVector[Double]): DiscreteField[D, Dom, Value] = {
    require(rank == c.size)
    val instVal = instanceVector(c)
    DiscreteField.createFromDenseVector[D, Dom, Value](domain, instVal)
  }
   ...
   */
  override def pdf(instance: DiscreteField[D, Dom, Value]): Double = pdf(coefficients(instance))
   ...
   * If you are interested in ordinal comparisons of PDFs, use this as it is numerically more stable
   */
  override def logpdf(instance: DiscreteField[D, Dom, Value]): Double = logpdf(coefficients(instance))

  /**
   * Discrete version of [[DiscreteLowRankGaussianProcess.sample]]
   */
  override def sample()(implicit random: Random): DiscreteField[D, Dom, Value] = {
   ...
   */
  def klBasis: KLBasis[D, Dom, Value] = {
    for (i <- 0 until rank) yield {
      val eigenValue = variance(i)
      val eigenFunction = DiscreteField.createFromDenseVector[D, Dom, Value](domain, basisMatrix(::, i))
  ...
  def truncate(rank: Int) = {
    DiscreteLowRankGaussianProcess[D, Dom, Value](mean, klBasis.take(rank))
  }

  /**
   * Discrete version of [[LowRankGaussianProcess.project(IndexedSeq[(Point[D], Vector[DO])], Double)]]
   */
  override def project(s: DiscreteField[D, Dom, Value]): DiscreteField[D, Dom, Value] = {
   ...
   */
  def coefficients(s: DiscreteField[D, Dom, Value]): DenseVector[Double] = {
   ...
   *
   */
  def posterior(trainingData: IndexedSeq[(PointId, Value)], sigma2: Double): DiscreteLowRankGaussianProcess[D, Dom, Value] = {
   ...
   *
   */
  def posterior(trainingData: IndexedSeq[(PointId, Value, MultivariateNormalDistribution)]): DiscreteLowRankGaussianProcess[D, Dom, Value] = {
 ...
 *
 */
private[scalismo] class NearestNeighbourInterpolatedLowRankGaussianProcess[D <: Dim: NDSpace, Dom <: DiscreteDomain[D], Value](mean: Field[D, Value],
  klBasis: LowRankGaussianProcess.KLBasis[D, Value],
  discreteGP: DiscreteLowRankGaussianProcess[D, Dom, Value])(implicit vectorizer: Vectorizer[Value])
...
object DiscreteLowRankGaussianProcess {
  ...
  case class Eigenpair[D <: Dim, Dom <: DiscreteDomain[D], Value](eigenvalue: Double, eigenfunction: DiscreteField[D, Dom, Value])
   ...
   */
  def apply[D <: Dim: NDSpace, Dom <: DiscreteDomain[D], Value](domain: Dom, gp: LowRankGaussianProcess[D, Value])(implicit vectorizer: Vectorizer[Value]): DiscreteLowRankGaussianProcess[D, Dom, Value] = {
    ...
    new DiscreteLowRankGaussianProcess[D, Dom, Value](domain, m, lambdas, U)
  }

  def apply[D <: Dim: NDSpace, Dom <: DiscreteDomain[D], Value](mean: DiscreteField[D, Dom, Value],
    klBasis: KLBasis[D, Dom, Value])(implicit vectorizer: Vectorizer[Value]): DiscreteLowRankGaussianProcess[D, Dom, Value] = {
    ...
    val meanVec = DiscreteField.vectorize[D, Dom, Value](mean)
    ...
    for ((eigenPair, i) <- klBasis.zipWithIndex) yield {
      ...
      basisMat(::, i) := DiscreteField.vectorize[D, Dom, Value](phi)
   ...
   */
  def regression[D <: Dim: NDSpace, Dom <: DiscreteDomain[D], Value](gp: DiscreteLowRankGaussianProcess[D, Dom, Value],
    trainingData: IndexedSeq[(PointId, Value, MultivariateNormalDistribution)])(implicit vectorizer: Vectorizer[Value]): DiscreteLowRankGaussianProcess[D, Dom, Value] = {
   ...
   *
   */
  def createUsingPCA[D <: Dim: NDSpace, Dom <: DiscreteDomain[D], Value](domain: Dom,
    fields: Seq[Field[D, Value]])(implicit vectorizer: Vectorizer[Value]): DiscreteLowRankGaussianProcess[D, Dom, Value] = {
